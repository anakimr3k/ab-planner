    ...
    <div className="min-h-screen bg-[#f8fafc] flex">
      ...
      <aside className="w-64 bg-white border-r border-gray-100 p-6 flex flex-col fixed h-full z-10">
        ...
        <nav className="space-y-2 flex-1">
          <div className="flex items-center gap-3 px-4 py-3 bg-blue-50 text-blue-600 rounded-xl font-bold cursor-pointer">
            <LayoutDashboard size={18} /> Dashboard
          </div>
          <div className="flex items-center gap-3 px-4 py-3 text-gray-400 hover:bg-gray-50 rounded-xl font-semibold cursor-pointer group">
            <CheckCircle2 size={18} /> To-Do List
          </div>
        </nav>
        <div className="mt-auto pt-6 border-t border-gray-50 flex items-center gap-3">
            <UserButton />
            <span className="text-xs font-bold text-gray-500">Mon Compte</span>
        </div>
      </aside>
      ...
      <main className="flex-1 ml-64 p-8">
        <header className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-gray-500 mt-1 font-medium">Tes données sont synchronisées avec Neon.</p>
          </div>
        </header>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div onClick={() => document.getElementById('fileInput')?.click()} className="relative border-2 border-dashed rounded-3xl p-12 flex flex-col items-center justify-center transition-all cursor-pointer border-gray-100 bg-white hover:border-blue-300">
            ...
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
              <h3 className="text-xl font-bold text-gray-900 mb-6">Tes prochains examens</h3>
                ...
                <div className="space-y-3">
                  {initialExamens.map((ex) => (
                    <div key={ex.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-transparent hover:border-blue-100 transition-all">
                      ...
                      <button onClick={() => deleteExamen(ex.id)} className="text-gray-300 hover:text-red-500 p-2">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  ))}
              ...
              <div className="mt-8 pt-8 border-t border-gray-50 grid grid-cols-2 gap-4">
                ...
                <button onClick={handleAjouterExamen} disabled={isSaving} className="col-span-2 bg-gray-900 text-white rounded-2xl py-4 font-bold hover:bg-blue-600 transition-all flex items-center justify-center gap-3 disabled:opacity-50">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 text-center">
              ...
              <p className="text-[11px] text-gray-400 mt-4 italic">Synchronisé avec Neon</p>
            </div>
          </div>
        </div>
      </main>
